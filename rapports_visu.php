...
$_SESSION['currentPage'] = "rapports_visu";
...
$sql_categ='SELECT CatégoriesProduits, Ports, Devises
          FROM others
          ORDER BY id;';
$sqlRapportEnregistrer = 'INSERT INTO `rapports` (`id`, `code` ,`num` ,`projet`, `commandes`, `visitesClient`, `offres` , `remarques`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?);';
...
try { 
  $db = include 'db_mysql.php';
    ...
    $stmt4 = $db->prepare($sql_rapports);
    $stmt4->execute(array());
    $rapports = $stmt4->fetchAll();
    ...
    if(count($data)==1 and password_verify($_POST['mdpactu'], $data[0]['mdp']) and $_POST['newmdp']==$_POST['confirm']){
      ...
      echo "mdp incorect";
              ...
              if(isset($_POST['boxmail_email1']) or isset($_POST['boxmail_email2']) or isset($_POST['boxmail_1']) or isset($_POST['boxmail_2']) or isset($_POST['boxmail_3']) or isset($_POST['boxmail_4'])){
                ...
                sendmail($str,'Rapport','rapport.pdf',true,getRapportCode(true));
                $db = include 'db_mysql.php';
                       $stmtenr = $db->prepare($sqlRapportEnregistrer);
                       $str = "";
                       $opt = "";

                       /*$p = 0; `projet`, `commandes`, `visitesClient`, `offres` , `remarques`
                       for (; $p < count($projets) and $projets[$p]['id']!=$_GET['pr']; $p++);*/
                        $existRapp=false;
                      for ($er=0; $er < count($rapports); $er++) { 
                        if($rapports[$er]['code']==$_SESSION['currentRapport']){
                          $existRapp=true;
                          break;
                        }
                      }
                      if($existRapp==false)
                       $stmtenr->execute(array($_SESSION['currentRapport'],$_SESSION['numeroRapport'],$projets[$p]['code'],$_SESSION['commandes'],$_SESSION['visites'],$_SESSION['offres'],$_SESSION['remarques']));
...
<?php include("header.php"); ?> 
              ...
              <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4 padding-top-0">
                ...
                <div class="container justify-content-center">
                  <iframe src="<?php if(isset($_SESSION['currentRapport']) and $_SESSION['currentRapport']!="") echo "rapports/".$_SESSION['currentRapport'].".pdf";?>" width="900" height="800" align="middle"></iframe>
                </div>
