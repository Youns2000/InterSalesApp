...
for($j = 0; $j < count($_SESSION['panier']['libelleProduit']); $j++)
   {
      ...
      $pdf->Write(5,$_SESSION['pays'][intval($origine)]);
      $ORIGINES .= $_SESSION['pays'][intval($origine)]."/";
      $pdf->SetX(140);
      ...
      for ($option=0; $option < nbOptions($_SESSION['panier']['libelleProduit'][$j]); $option++) { 
        if(qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1)>0){
          ...
          $pdf->MultiCell(77,5,utf8_decode($_SESSION['options']['Nom'][$option]));
          $pdf->SetX(140);
          ...
          $pdf->Write(-5,utf8_decode($_SESSION['options']['Prix'][$option]).iconv("UTF-8", "CP1252", $DEVISE_SIGNE));
          $pdf->SetX(185);
          ...
          $pdf->Write(-5,floatval(utf8_decode($_SESSION['options']['Prix'][$option])*qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1)).iconv("UTF-8", "CP1252", $DEVISE_SIGNE));
          //$TOTALPRICE+=floatval(utf8_decode($options[$option]['Prix'])*qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1));
          $TOTALPRICE+=floatval(utf8_decode($_SESSION['options']['Prix'][$option])*qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1));
          //$TOTALTRANSPORT+=floatval(utf8_decode($options[$option]['prix_transport'])*qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1));
          $TOTALTRANSPORT+=floatval(utf8_decode($_SESSION['options']['prix_transport'][$option])*qteOption($_SESSION['panier']['libelleProduit'][$j],$option+1));
        }
      }
      $j++;
}
